...
class Deque(object):
    def __init__(self, title=None, author=None, n_pages=0, price=0,
                 category=None, pub_company=None, year=2017, id_b=0):
        self.title = title
        self.author = author
...
def is_empty(deque):
    return deque.next is None


# Inserir novo nó no início do deque
def add_first(deque):
    new_node = input_data()
    new_node.next = deque.next
    deque.next = new_node
...
# Inserir novo nó no final do deque
def add_last(deque):
    new_node = input_data()
